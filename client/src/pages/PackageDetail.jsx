          ...
          <div className="flex-1 space-y-8">
            ...
            {pkg.paymentDetails && (pkg.paymentDetails.upiId || pkg.paymentDetails.accountNumber) && (
              <section className="bg-white rounded-xl border border-surface-container-high p-5 shadow-soft">
                <h2 className="text-xl font-bold text-primary mb-4 flex items-center gap-2">
                  <span className="material-symbols-outlined text-accent font-filled">payments</span>
                  Payment Details
                </h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-2">
                  {pkg.paymentDetails.upiId && (
                    <div className="flex justify-between items-center py-2.5 border-b border-surface-container-high">
                      <span className="text-sm font-semibold text-outline">UPI ID</span>
                      <span className="text-sm font-bold text-primary font-mono select-all bg-accent/5 text-accent px-2.5 py-1 rounded-lg border border-accent/10">{pkg.paymentDetails.upiId}</span>
                    </div>
                  )}
                  {pkg.paymentDetails.upiPhoneNumber && (
                    <div className="flex justify-between items-center py-2.5 border-b border-surface-container-high">
                      <span className="text-sm font-semibold text-outline">UPI Phone Number</span>
                      <span className="text-sm font-bold text-primary">{pkg.paymentDetails.upiPhoneNumber}</span>
                    </div>
                  )}
                  {pkg.paymentDetails.bank && (
                    <div className="flex justify-between items-center py-2.5 border-b border-surface-container-high">
                      <span className="text-sm font-semibold text-outline">Bank Name</span>
                      <span className="text-sm font-bold text-primary">{pkg.paymentDetails.bank}</span>
                    </div>
                  )}
                  {pkg.paymentDetails.accountNumber && (
                    <div className="flex justify-between items-center py-2.5 border-b border-surface-container-high">
                      <span className="text-sm font-semibold text-outline">Account Number</span>
                      <span className="text-sm font-bold text-primary font-mono select-all bg-surface-container px-2 py-1 rounded">{pkg.paymentDetails.accountNumber}</span>
                    </div>
                  )}
                  {pkg.paymentDetails.ifsc && (
                    <div className="flex justify-between items-center py-2.5 border-b border-surface-container-high">
                      <span className="text-sm font-semibold text-outline">IFSC Code</span>
                      <span className="text-sm font-bold text-primary font-mono select-all bg-surface-container px-2 py-1 rounded">{pkg.paymentDetails.ifsc}</span>
                    </div>
                  )}
                </div>
              </section>
